       ...
       static void Main(string[] args)
        {
            ...
            {
                //Variables de l'IA
                IAechec chessMaster = new IAechec();
                bool promotion;
                int depth = 3;
                int bestValue;
                String[] bestMove;
                    ...
                    using (var mmf = MemoryMappedFile.OpenExisting("plateau"))
                    {
                        using(var mmf2 = MemoryMappedFile.OpenExisting("repAI1"))
                        {
                            ...
                            if (!stop)
                            {
                                ...
                                chessMaster.InitEchiquier(tabVal, tabCoord);

                                //Algo MiniMax
                                bestValue = chessMaster.MiniMax(depth, -10000, 10000);

                                //Réinitilisation de la couleur par défaut de l'IA
                                chessMaster.ResetColorIA();

                                //Renverse le Best Move si IA noir
                                chessMaster.ReverseBestMove(tabCoord);

                                //Meilleur move trouvé
                                bestMove = chessMaster.GetBestMove();

                                //Vérification du cas d'une promotion de pièces
                                promotion = chessMaster.IsPromoted();

                                //Envoi au plateau (0 = depart, 1 = arrivé, 2 = promotion)
                                coord[0] = bestMove[0];
                                coord[1] = bestMove[1];
                                if (promotion) coord[2] = "D"; //IA récupère une reine !!
